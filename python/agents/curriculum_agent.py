...
class CurriculumAgent(BaseAgent):
    ...
    def set_knowledge_graph(self, graph: KnowledgeGraph) -> None:
        ...
        logger.info("[CurriculumAgent] Knowledge graph set with %d nodes", len(graph.nodes))
    ...
    async def _check_and_recommend_next(
        ...
        if not self.knowledge_graph:
            logger.warning("[CurriculumAgent] Knowledge graph not set, skipping recommendation")
            return
    ...
    async def _handle_weakness(self, event: Event) -> None:
        """处理薄弱知识点，规划补救路径。"""
        if not self.knowledge_graph:
            logger.warning("[CurriculumAgent] Knowledge graph not set, skipping weakness handling")
            return
